...
const DeleteChatButton = ({ children ,id}: { children: React.ReactNode,id:Id<"personal_chat"> }) => {
  const [open, setOpen] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [isPending, startTransition] = useTransition()

  const DeleteWebsite = useMutation(api.website.client.DeleteChat.default)
    ...
    <Dialog open={open} onOpenChange={(val) => !isPending && setOpen(val)}>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Warning this will delete chat permanently no backup 
          </DialogDescription>
        </DialogHeader>
   {error && (
