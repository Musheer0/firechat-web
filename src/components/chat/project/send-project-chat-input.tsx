...
const SendProjectChatMessageInput = ({ id }: { id: Id<"project"> }) => {
  const SendMessage = useAction(api.project.client.SendProjectMessage.default)
  ...
  const handleSend = useCallback(() => {
    ...
    startTransition(async () => {
      try {
        ...
        await SendMessage({project_id:id,message:message});
        setMessage("")
      } catch (err: any) {
  ...
  return (
    ...
    >
      <TextareaAutosize
        minRows={2}
        maxRows={6}
        placeholder="Ask anything about the  websites..."
        value={message}
